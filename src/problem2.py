...
def problem2b(rect, n, delta, win):
    ...
    # -------------------------------------------------------------------------
    rect.attach_to(win)
    left = rect.get_lower_left_corner()
    right = rect.get_upper_right_corner()
    for k in range(n):
        rect = rg.Rectangle(rg.Point(left.x-delta,left.y + delta),rg.Point(right.x+delta,right.y-delta))
        left = rect.get_lower_left_corner()
        right = rect.get_upper_right_corner()
        rect.attach_to(win)
        win.render()
